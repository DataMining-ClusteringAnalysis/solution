...
FROM (SELECT *, (CASE WHEN EXTRACT(DOW FROM _ts) = 1 THEN 'Mon'
                 ...
                 END) AS dayOfWeek
      FROM trips LEFT JOIN cities on trips.city_id = cities.city_id LEFT JOIN events on trips.client_id = events.rider_id
      WHERE EXTRACT(YEAR FROM _ts) = 2016 AND
            EXTRACT(WEEK FROM _ts) = 1 AND
